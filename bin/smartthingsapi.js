...
var smartthings = {
    ...
    getAttribute: function (deviceid, attribute, callback) {
        GET({ debug: false, path: deviceid + '/attribute/' + attribute }, function (data) {
            if (data) {
                if (callback) { callback(data); callback=undefined; };
            } else {
                if (callback) { callback(); callback=undefined; };
    ...
    setAttribute: function (callback, deviceid, attribute, value) {
        PUT({ debug: false, path: deviceid + '/attribute/' + attribute + '/' + value }, function (data) {
            if (data) {
                if (callback) { callback(data); callback=undefined; };
            } else {
                if (callback) { callback(); callback=undefined; };
